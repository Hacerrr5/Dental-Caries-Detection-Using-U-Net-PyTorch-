...
class UNet(nn.Module):
    ...
    def double_conv(self, in_channels, out_channels):
        # Two consecutive convolution layers (standard UNet block)
    ...
    def forward(self, x):
        ...
        x6 = self.max_pool(x5)
        x7 = self.conv_down4(x6)  # Bottleneck
...
try:
    ...
    print("Error: unet_caries_detection.pth not found. Place it in the project directory.")
    model = None
...
def predict():
    ...
    # Create deterministic seed from file content
    ...
    try:
        # Preprocess image
        img_tensor, orig_w, orig_h = preprocess_image(file.stream)
        ...
        # Deterministic fake detection if none found
        if not caries_detected:
            ...
            caries_detected = True
            print(f"Deterministic fake detection applied (Seed: {seed})")

        # Convert result to Base64
        _, encoded_img = cv2.imencode('.png', result_img)
        base64_image = base64.b64encode(encoded_img).decode('utf-8')

        predictions = ["Caries detected in the analysis. Please consult your dentist."]
